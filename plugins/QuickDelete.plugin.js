...
QuickDelete.prototype.convert = function () {
    $('div.message-text').each(function (i, el) {
        ...
        if (current_msg.find('div.btn-option').length == 1 && current_msg.find('div.remove_msg').length == 0) {
            current_msg.find('div.btn-option').after('<div class="remove_msg" style="visibility: hidden;padding-right: 10px; background: url(\'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAABAAAAAQCAYAAAAf8/9hAAAAXUlEQVR42u2SwQoAIAhD88vVLy8KBlaS0i1oJwP3piGVg0Skmpq8HjqZrWl9uwCbGAmwKYGZs/6iqgMyAdJuM8W2QmYKpLt/0AG9ASCv/oAnANd3AEjmAlFT1BypAV+PnRH5YehvAAAAAElFTkSuQmCC\') 50% no-repeat; width: 16px; height: 16px; background-size: 16px 16px; cursor: pointer; float: right;"></div>');
        }
    });
};
...
try {
    ...
    console.warn('Using old version, not exporting functions.');
}